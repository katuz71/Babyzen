    ...
    <ScreenWrapper style={{ backgroundColor: '#0B0E14' }}>
      <ScrollView className="flex-1 px-5">
        <View className="mt-6 mb-8 flex-row justify-between items-end">
          <View>
            <Text className="text-gray-400 text-sm uppercase tracking-widest font-bold">Zen Library</Text>
            <Text className="text-3xl font-bold text-white">Спокойный сон</Text>
          </View>
          {secondsLeft !== null && (
            <View className="bg-[#D00000] px-3 py-1 rounded-full mb-1">
        ...
        <View className="flex-row flex-wrap justify-between">
          {SOUNDS_LIST.map((item) => (
            ...
            >
              <LinearGradient
                colors={playingId === item.id ? ['#2D1B4E', '#161B22'] : ['#161B22', '#161B22']}
                className={`rounded-3xl p-6 items-center border ${
                  playingId === item.id ? 'border-[#D00000]' : 'border-gray-900'
                }`}
              >
                <View 
                ...
                >
                  {loadingId === item.id ? (
                    ...
                    <Ionicons 
                      ...
                      size={32} 
                      color={playingId === item.id ? '#D00000' : item.color} 
                    />
                  )}
                </View>
                <Text className={`font-bold text-center text-sm ${playingId === item.id ? 'text-white' : 'text-gray-400'}`}>
                  {item.title}
                </Text>
              </LinearGradient>
            </TouchableOpacity>
        ...
        <View className="mt-4 mb-10">
          <Text className="text-white font-bold mb-4 text-lg">Таймер выключения</Text>
          <View className="flex-row justify-between">
            {TIMER_OPTIONS.map((min) => (
              <TouchableOpacity
                ...
                className={`px-4 py-3 rounded-2xl border ${
                  secondsLeft === min * 60 ? 'bg-[#D00000] border-[#D00000]' : 'bg-[#161B22] border-gray-800'
                }`}
              >
                <Text className={`font-bold ${secondsLeft === min * 60 ? 'text-white' : 'text-gray-400'}`}>
                  {min} м
                </Text>
              </TouchableOpacity>
            ))}
            <TouchableOpacity
              ...
              className="px-4 py-3 rounded-2xl bg-[#161B22] border border-gray-800"
            >
